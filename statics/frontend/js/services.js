var translateObject={
    english:{
        services:"OUR SERVICES",
        servicesTitle:["Airport Service","Airport Service","Battery Boost","Battery Boost","Unlock A Door","Unlock A Door","Delivery","Delivery","Corporative Account","Corporative Account","Event Transportation","Event Transportation","Don't Drink and Drive","Don't Drink and Drive","School Transportation","School Transportation"],
        servicesText:[
        `Taxi Bonjour Montreal, our taxi services are particularly used to take our customers to Montreal Pierre-Elliott-Trudeau Airport (YUL)., Booking a reservation is guaranteed, an economical solution that saves you the cost of airport parking. Avoid the shuttle and get off directly at your departure gate!
        For more info, contact us today. `,
        `Can’t start your car? No worries. We will be there in no time to jump start a dead battery in your car.Our drivers are, for the most part, equipped with jump cables. Some also have a mobile booster device Our quick and efficient service is especially appreciated during times of extreme, cold weather conditions, but is offered year-round`,
        `If you lock your key inside the car and you need help to unlcok the door or your car battery is out of juice, No problem – we are equipped to help you. With Taxi VIP St-Hubert, our qualified chauffeurs are ready to unlock your car doors, quickly and easily`,
        `We deliver your item to destination or we pick up at a location and deliver to your place, Taxi VIP St-Hubert offers a fast and safe delivery service for small parcels over short distances.`,
        `<span class="fs-5 theme-color">Simplify your life!</span>
        you can call a taxi for your business or for your family under your Atlas account and just pay the fare by monthly bill, Do like hundreds of Montreal businesses and become a Taxi VIP St-Hubert corporate client! Easy to use and totally safe`,
        `Offer secure transportation for your guests/members/participants. We can organize  all the transportation logistics needed during/for your event, be it a convention, workshop, etc. From 10-300 people, Taxi Coop is there for you! `   ,
        `<span class="fs-5 theme-color">Been drinking? Don’t drive! Call Taxi VIP St-Hubert! </span>
        There’s no reason to get behind the wheel. If you’ve been drinking, you put yourself and others in danger! We offer transportation 24 hours a day / 365 days a year we also offer debit and credit card services, Our vehicles are comfortable and spacious enough for you and your companions. `,
        `<span class="fs-5 theme-color">THERE ARE MANY ADVANTAGES TO USING THIS SERVICE</span>
        You can plan the itinerary in advanceA fixed rate, established in advance Transportation that’s safe, quick, and flexiblePossibility of having the same driver for repeat trips
` 
    ],
        learnMoreBtn:["Learn More","Learn More","Learn More","Learn More","Learn More","Learn More","Learn More","Learn More"],
        closeBtn:["close","close","close","close","close","close","close","close"],
        additionalServicesTitle:"Additional services",
        additionalServicesText:`Taxi VIP St-Hubert also offers a number of services tailored to your needs.
                                Here's how he can help you:`,
        additionalServices:["Medical transport (SAAQ, CSST)","Group transportation (6-passenger van)","Baby seat available on request"," All the cars in Atlas taxi are always clean and air conditioned and pet friendly"],
        photos:{
            corporateService:"corporate_travelُُُENGLISH.jpg" ,
            drunkService: "DRUNKENG.jpg"
        } 
    },
     french:{
        services:"NOS SERVICES",
        servicesTitle:["Transport vers l'aéroport","Transport vers l'aéroport ","Déverrouillage des portières / survoltage de batterie","Déverrouillage des portières / survoltage de batterie","Service de déverrouillage de portière","Service de déverrouillage de portière","Livraison","Livraison","Comptes Corporatifs ","Comptes Corporatifs ","Événements transport","Événements transport","Ne conduisez pas sous l'emprise de l'alcool","Ne conduisez pas sous l'emprise de l'alcool","Transport Scholaire","Transport Scholaire"],
        servicesText:[
            `Chez Taxi VIP St-Hubert, nos services de taxi sont particulièrement utilisés pour mener notre clientèle vers l’aéroport de Montréal Pierre-Elliott-Trudeau (YUL), garantit toutes vos réservations de courses pour l’aéroport, une solution économique qui vous épargne les frais de stationnement de l'aéroport. Évitez la navette et débarquez directement à la porte des départs!
             Pour obtenir plus de renseignements, contactez-nous dès aujourd’hui.`,
            `Votre voiture ne démarre plus? Pas de souci. Nous nous déplacerons en en moins de deux pour survolter la batterie à plat de votre voiture. nos chauffeurs sont, pour la plupart, équipés de câbles de survoltage. Certains possèdent également un appareil de survoltage mobile, Notre service rapide et courtois de survoltage vous évitera les interminables attentes lors des froides journées d’hiver. Ce service est disponible en tout temps.
             Pour obtenir plus de renseignements, contactez-nous dès aujourd’hui.`,
            `Vous avez perdu votre clé de voiture ou sa batterie est à plat, Pas de problème. Nous sommes équipés pour vous aider, Avec Taxi VIP St-Hubert, nos chauffeurs qualifiés pourront déverrouiller vos portières rapidement et sans tracas.
             Pour obtenir plus de renseignements, contactez-nous dès aujourd’hui`,
            `Nous livrons vos articles à destination ou nous les récupérons à un endroit désigné, pour ensuite les livrer à votre place, Taxi VIP St-Hubert offre un service de livraison rapide et sécuritaire pour vous aider à envoyer vos petits colis à courte distance.
             Pour obtenir plus de renseignements, contactez-nous dès aujourd’hui.`,
            `<span class="fs-5 theme-color"> Un système qui vous simplifiera la vie </span>
            Vous pouvez, à partir de votre compte, appeler un taxi pour votre entreprise ou pour votre famille. Il vous suffit simplement de payer votre facture mensuelle calculée à partir du total de vos coupons, Faites comme des centaines d’entreprises montréalaises et devenez client corporatif de Taxi VIP St-Hubert! Facile à gérer et sécurisé, 
            Pour obtenir plus de renseignements, contactez-nous dès aujourd’hui.`,
            `Pour déplacer vos invités en toute sécurité, nous pouvons vous aider à organiser la logistique du transport lors de vos événements, congrès, séminaires, etc. Que ce soit pour 10, 50 ou 300 personnes, Taxis Coop est là pour vous!
            Pour obtenir plus de renseignements, contactez-nous dès aujourd’hui.`,
            `<span class="fs-5 theme-color">Vous avez été alcoolisé(e) ? Ne conduisez pas ! Appelez Taxi VIP St-Hubert !</span>
            Il n'y a aucune raison de prendre le volant. Si vous avez consommé de l'alcool, vous vous mettez en danger et vous mettez les autres en danger ! Nous proposons des transports 24 heures sur 24 et 365 jours par an, ainsi que des services de cartes de débit et de crédit. Nos véhicules sont confortables et suffisamment spacieux pour vous et vos compagnons
            Pour obtenir plus de renseignements, contactez-nous dès aujourd’hui`,
            `<span class="fs-5 theme-color">L’UTILISATION DE CE SERVICE OFFRE PLUSIEURS AVANTAGES</span>
            Planification du trajet à l’avanceTarif fixe, communiqué au préalableTransport sécuritaire, rapide et flexiblePossibilité d’avoir le même chauffeur de confiance pour vos trajets répétés`
        ],
        learnMoreBtn:["En savoir plus","En savoir plus","En savoir plus","En savoir plus","En savoir plus","En savoir plus","En savoir plus","En savoir plus"],
        closeBtn:["fermer","fermer","fermer","fermer","fermer","fermer","fermer","fermer"],
        additionalServicesTitle:"Services additionnels",
        additionalServicesText:`Taxi VIP St-Hubert propose aussi  plusieurs services adaptés à vos besoins.
                                Voici comment il peut vous assister :`,
        additionalServices:["Transport médical (SAAQ, CSST)","Transport de groupe (fourgonnette de 6 passagers)","Siège de bébé disponible sur demande","Les voitures de Taxi VIP St-Hubert sont toujours propres et climatisées, en plus d’accepter les animaux domestiques"],
        photos:{
            corporateService:"corporate_travel .jpg" ,
            drunkService: "Drunk fr2.jpg"
        } 
     }
}
    translate(localStorage.getItem('language'))